...
public class TestCoverageAnalyzer : DiagnosticAnalyzer
{
        ...
        Compilation compilation)
    {
        ...
        foreach (var type in GetAllTypes(compilation.GlobalNamespace))
        {
            ...
            if (ShouldSkipSourceType(sourceType))
            {
                continue;
            }
    ...
    private static bool ShouldSkipSourceType(INamedTypeSymbol type)
    {
        var ns = type.ContainingNamespace?.ToDisplayString() ?? string.Empty;
        if (type.Name == "GameBridge" || type.Name == "MockGameBridge")
        {
            return true;
        }

        if (ns.Contains(".Models") || ns.Contains(".Events"))
        {
            return true;
        }
