    ...
    private static void AnalyzeConstructor(SyntaxNodeAnalysisContext context)
    {
        ...
        if (constructor.Parent is TypeDeclarationSyntax containingType)
        {
            ...
            if (typeSymbol != null)
            {
                ...
                if (ns.Contains(".Models")
                    || ns.Contains(".Events")
                    || typeSymbol.Name.EndsWith("EventArgs")
                    || typeSymbol.Name.EndsWith("Row")
                    || typeSymbol.Name.EndsWith("Data")
                    || typeSymbol.Name.EndsWith("Snapshot")
                    || typeSymbol.Name.EndsWith("Config")
                    || typeSymbol.Name.EndsWith("Info"))
                {
                    return;
                }
            }
